function run()
{
usage="Usage: run-motivation.sh <full_qualified_class_name> <ssn| eg. 7|8|9|10|11|12> <o eg. true|false> <cn eg. 5>"
if [ $# -lt 4 ];then
        echo "$usage"
        exit 1
else
        ...
        equal="==========================="
        log_dir="${SPARK_HOME}/logs/JsonCost-$ssn-$o-$class_name-$time_now.log"
        ...
        exec_command="nohup ./bin/spark-submit --class $full_qualified_class_name --driver-cores 2 --driver-memory 8g --master yarn --deploy-mode cluster  --num-executors 4  --executor-memory 8g --executor-cores 1 --conf spark.app.name=JsonCost-$ssn-$o  --conf spark.sql.catalogImplementation=hive --conf spark.json.optimize.cost=true ${jar_path} -ssn $ssn -o $o -cn $cn > $log_dir 2>&1 &"
...
_usage="Usage: run-motivation.sh <ssn| eg. 7|8|9|10|11|12> <o eg. true|false> <cn eg. 5>"
if [ $# -lt 3 ];then
        echo "$_usage"
        exit 1
fi
_full_qualified_class_name="com.fox.examples.sql.motivation.JsonCost"
_ssn=$1
_o=$2
_cn=$3
